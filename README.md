...
| **BactCG** | Core genome identification | Protein FASTA files directory | Core gene clusters + MEG alignments + SNP data |
...
```
Phylogenetic Tree + Genome Sequences
    │
    ├─ [1] BactAG ──→ Ancestral Genomes (AOGs) + bactID.txt
    │                    │
    ├─ [2] BactCG ──→ Core Genes (parallel)
    │                    │
    ├─ [3] BactPG ──→ Pan-Genome Matrix (parallel)
    │                    │
    ├─ [4] BactPGA ──→ Annotated Gene Tables
    │
    ├─ [5] Bact1DGR ──→ 1D Genome Representations
    │                    │
    │                    └─→ [7] BactFragAnn (1DGR mode) ──→ Interactive HTML
    │
    └─ [6] BactEvolTraj ──→ SV Analysis
                         │
                         └─→ [7] BactFragAnn (EvolTraj mode) ──→ Interactive HTML
```
...
```bash
# The project includes a pre-built environment
...
│   ├── index.html                     # Static entry page (direct access)
│   ├── app.py                         # Flask app with sub-path support
...
- **Homepage**: [https://tools.szu-bioinf.org/GenomEvo/](https://tools.szu-bioinf.org/GenomEvo/)
