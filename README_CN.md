...
├── Genomevo_web/                      # 独立Web部署版
│   ├── index.html                     # 静态入口页（直接访问）
│   ├── app.py                         # 支持子路径部署的Flask应用
│   ├── .htaccess                      # Apache mod_rewrite规则
│   └── templates/                     # Web界面模板
└── workflow.svg                       # 主流程图
```
...
- **王冶俊（PI）** – wangyj@szu.edu.cn
- **胡月明** – huym@fynu.edu.cn
- 王一苇、陈培俊、郑敏、邓志轩、赵秀哲、袁铭、彭天瑞、郭鑫蔓

深圳大学医学部 医学生物信息学青年创新团队
